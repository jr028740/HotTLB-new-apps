...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void PrepareReadRandom2() {
    ...
    if (db_.db == nullptr || !multi_dbs_.empty()) {
      fprintf(stderr, "readrandom2 requires exactly one RocksDB database\n");
      ErrorExit();
    }
    if (FLAGS_num <= 0 || FLAGS_value_size <= 0) {
      fprintf(stderr, "readrandom2 requires positive --num and --value_size\n");
      ErrorExit();
    }
    if (user_timestamp_size_ > 0) {
      fprintf(stderr, "readrandom2 does not support user timestamps\n");
      ErrorExit();
    }

    readrandom2_db_ = db_.db;
    ...
    if (readrandom2_entries_ >
        static_cast<uint64_t>(std::numeric_limits<size_t>::max() /
                              readrandom2_stride_)) {
      fprintf(stderr, "readrandom2 working-set size overflow\n");
      ErrorExit();
    }
    ...
    fprintf(stderr, "Starting %d parallel RocksDB value loaders\n",
            loader_threads);
    fflush(stderr);
...
// optimization, so skip the check of the max level.
  ...
  void ReadRandom2(ThreadState* thread) {
    if (readrandom2_data_ == nullptr || readrandom2_db_ == nullptr ||
        readrandom2_entries_ == 0) {
      fprintf(stderr, "readrandom2 working set is not prepared\n");
      ErrorExit();
    }
    ...
    if (hot_count == 0) {
      hot_count = shard_size;
    }

    uint64_t thread_reads =
        static_cast<uint64_t>(reads_) / thread_count;
    ...
    if (thread_id == 0) {
      fprintf(stderr,
              "signalling readyness to /tmp/enablement/rocksdb_watch\n");
      FILE* ready_file = fopen("/tmp/enablement/rocksdb_watch", "w+");
      if (ready_file == nullptr) {
        fprintf(stderr, "ERROR: could not create RocksDB readiness file\n");
      } else {
