...
// optimization, so skip the check of the max level.
  ...
  void ReadRandom2(ThreadState* thread, DB* db) {
    ...
    //	getchar();
    fprintf(stderr, "signalling readyness to /tmp/enablement/rocksdb_watch\n");
    FILE *fd2 = fopen("/tmp/enablement/rocksdb_watch", "w+");
    ...
    for ( int j = 0; j < 2; ++j) {
	  ...
	  lfsr = lfsr_fast(lfsr);
          ...
          access_idx = lfsr % (int64_t(0.2 * FLAGS_num)) + int64_t(0.7 * FLAGS_num);
        } else {
	  lfsr = lfsr_fast(lfsr);
