...
PMDMON_START="$HOTTLB_DIR/pmdmon_start_hottlb_lmdb"
PMDMON_STOP="$HOTTLB_DIR/pmdmon_stop"

OUTPUT_FILE="$DATAPATH/lmdb.out.$DATETIME.hottlb"
...
echo "LMDB HotTLB started with PID $LMDB_PID"
...
datapath="$DATAPATH" \
datetime="$DATETIME" \
"$PERF_SCRIPT" lmdb-hottlb "$LMDB_PID" &
...
echo "LMDB HotTLB exit status: $LMDB_STATUS"
