...
void launch_perf(int cpid, const char* perf_cmd) {
  ...
  int err = execl(perf_cmd, "perf_command.sh", "hottlb", pid_str, NULL);
...
void BenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  const char *perf_cmd = "/home/l1/Documents/HotTLB-new-apps/bench_gapbs_cc/perf_command.sh";
  ...
  for (int iter=0; iter < cli.num_trials(); iter++) {
	...
	setpgid(cpid, cpid); // Fix the setpgid race condition
	usleep(500000);
