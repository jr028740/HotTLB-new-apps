...
PMDMON_START="$GEMINI_DIR/pmdmon_start_gemini_svm"
PMDMON_STOP="$GEMINI_DIR/pmdmon_stop"

DATASET="$DATAPATH/data/test2-10gb"
OUTPUT_FILE="$DATAPATH/svm.out.$DATETIME.gemini"
...
echo "SVM Gemini started with PID $SVM_PID"
...
datapath="$DATAPATH" \
datetime="$DATETIME" \
"$PERF_SCRIPT" svm-gemini "$SVM_PID" &
...
echo "SVM Gemini exit status: $SVM_STATUS"
