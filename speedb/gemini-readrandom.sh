...
PMDMON_START="$GEMINI_DIR/pmdmon_start_gemini_speedb"
...
echo "Speedb Gemini started with PID $DBBENCH_PID"
...
datapath="$DATAPATH" \
datetime="$DATETIME" \
"$PERF_SCRIPT" speedb-gemini "$DBBENCH_PID" &
...
echo "Speedb Gemini exit status: $DBBENCH_STATUS"
echo "Output: $OUTPUT_FILE"
echo "Done."
