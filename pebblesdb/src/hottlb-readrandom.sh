...
PMDMON_START="$HOTTLB_DIR/pmdmon_start_hottlb_pebblesdb"
...
echo "PebblesDB HotTLB started with PID $DBBENCH_PID"
...
datapath="$DATAPATH" \
datetime="$DATETIME" \
"$PERF_SCRIPT" pebblesdb-hottlb "$DBBENCH_PID" &
...
echo "PebblesDB HotTLB exit status: $DBBENCH_STATUS"
