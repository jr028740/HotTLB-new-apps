...
OUTPUT_FILE="$DATAPATH/readrandom.out.$DATETIME.thp"
...
echo "PebblesDB THP started with PID $DBBENCH_PID"
...
datapath="$DATAPATH" \
datetime="$DATETIME" \
"$PERF_SCRIPT" pebblesdb-thp "$DBBENCH_PID" &
...
echo "PebblesDB THP exit status: $DBBENCH_STATUS"
